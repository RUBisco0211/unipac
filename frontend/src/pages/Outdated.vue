...
const uninstallTarget = ref<Package | null>(null)
...
const filteredPackages = computed(() => {
    ...
    return packages.value.filter(pkg => {
        ...
        return pkg.outdated && matchesManager && matchesQuery
    })
})
...
async function confirmUninstall() {
    const pkg = uninstallTarget.value
    if (!pkg) return
    showUninstallDialog.value = false
    uninstallTarget.value = null
...
<template>
    <div class="h-full overflow-auto">
        <div class="mx-auto max-w-6xl px-4 py-4 text-[13px]">
            <section>
                <h1 class="text-[28px] font-semibold tracking-tight">{{ t('outdated.title') }}</h1>
            </section>
    ...
    <Dialog v-model:open="showUninstallDialog">
        <DialogContent class="gap-0">
            <DialogHeader class="border-b-0">
                <DialogTitle>{{ t('packages.confirmUninstallTitle') }}</DialogTitle>
                <DialogDescription>
                    {{ t('packages.confirmUninstallDesc', { name: uninstallTarget?.name ?? '' }) }}
                </DialogDescription>
            </DialogHeader>
            <DialogFooter class="border-t-0">
                <Button variant="outline" @click="cancelUninstall">
                    {{ t('search.cancel') }}
                </Button>
